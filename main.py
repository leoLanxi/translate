...
# -*- coding: utf-8 -*-
"""
Japanese Video Subtitle Generator v3.0 (Multi-language + GPU Acceleration + Progress Display)
===============================================================================================

Features:
    1. Batch process video files (supports mp4, mkv, avi, mov)
    2. Japanese speech recognition using faster-whisper model
    3. Support generating Japanese/English/Chinese subtitles (optional)
    4. Auto-detect GPU/CPU, Windows CUDA acceleration
    5. Real-time progress bar display
    6. Support resume from breakpoint (can continue after interruption)
    7. Optional subtitle burning (hard subtitles) to video
...
Install dependencies:
    ...
    # Windows CUDA acceleration additional installation (optional):
    pip install nvidia-cublas-cu12 nvidia-cudnn-cu12

Run:
    python main.py

Note: This is a no-burn version (subtitle burning disabled by default).
      Set BURN_SUBTITLE = True and install ffmpeg to enable video burning.
"""
...
# ==================== CUDA DLL 路径配置（Windows）====================
# 自动添加 nvidia cuDNN/cuBLAS DLL 路径，解决 cudnn_ops64_9.dll 找不到的问题
if sys.platform == "win32":
    try:
        nvidia_path = os.path.join(site.getusersitepackages(), "nvidia")
        if os.path.exists(nvidia_path):
            # 添加 cuDNN 和 cuBLAS 的 bin 目录到 DLL 搜索路径
            cudnn_bin = os.path.join(nvidia_path, "cudnn", "bin")
            cublas_bin = os.path.join(nvidia_path, "cublas", "bin")
            
            for dll_path in [cudnn_bin, cublas_bin]:
                if os.path.exists(dll_path):
                    os.add_dll_directory(dll_path)
                    os.environ["PATH"] = dll_path + os.pathsep + os.environ.get("PATH", "")
    except Exception as e:
        pass  # 如果失败，继续运行，可能会使用 CPU 模式

# ==================== HuggingFace 镜像配置（中国大陆用户）====================
# 设置 HuggingFace 镜像源，加速模型下载
...
# Whether to burn subtitles into video (generate new video with hard subtitles)
# Note: Requires ffmpeg to use this feature
# If ffmpeg is not installed, set to False, program will still generate .srt subtitle files
# This is a no-burn version - set to False by default
BURN_SUBTITLE = False  # Set to True requires ffmpeg installation
...
def detect_device() -> Tuple[str, str]:
    ...
    Returns:
        ...
        - CPU: ("cpu", "int8")
    """
    # 首先检查 nvidia-smi（Windows上最可靠的方法）
    try:
        if sys.platform == "win32":
            # Windows上使用完整路径或直接命令
    ...
    if platform.processor() == 'arm' or 'Apple' in platform.processor():
        ...
        if sys.platform == "win32":
            print("  提示：如果您的电脑有NVIDIA显卡，请确保已安装NVIDIA驱动")
    return "cpu", "int8"
    ...
    if BURN_SUBTITLE:
        ...
        if ffmpeg_path:
            ...
            if sys.platform == "darwin":
                ...
                print("  或者使用 Chocolatey: choco install ffmpeg")
                print("  或者使用 winget: winget install ffmpeg")
            else:
...
def load_whisper_model(model_size: str = MODEL_SIZE):
    ...
    print("     模型下载可能需要几分钟，请耐心等待...")
    
    # 自动检测设备
    device, compute_type = detect_device()
    
    try:
        print(f"正在初始化模型 (设备: {device}, 精度: {compute_type})...")
        model = WhisperModel(
            model_size,
            device=device,
        ...
        if "CUDA" in str(e) or "cuda" in str(e).lower():
            print("  提示：CUDA 相关错误，尝试使用 CPU 模式...")
            try:
                model = WhisperModel(
                    model_size,
                    device="cpu",
...
def burn_subtitles_with_progress(
    ...
    # 处理路径（Windows 兼容）
    if sys.platform == "win32":
        # Windows 下需要特殊处理路径：将反斜杠转换为正斜杠，并转义特殊字符
        # 使用绝对路径并转义
        abs_srt_path = os.path.abspath(srt_path).replace("\\", "/")
        # 转义单引号和冒号
        srt_path_escaped = abs_srt_path.replace("'", r"\'").replace(":", r"\:")
    else:
...
def main():
    ...
    ║     (No-burn version - SRT files only)                      ║
    ║                                                            ║
    ║     🚀 GPU Acceleration | 📊 Real-time Progress | 💾 Resume║
    ╚════════════════════════════════════════════════════════════╝
    """)
    
    # Subtitle language name mapping
    ...
    print(f"  • Subtitle language: {lang_name} ({SUBTITLE_LANGUAGE})")
    print(f"  • Burn subtitles: {'Yes' if BURN_SUBTITLE else 'No (SRT only)'}")
    print()
    
    # Validate subtitle language setting
    if SUBTITLE_LANGUAGE not in ["ja", "en", "zh"]:
        print(f"✗ Error: Unsupported subtitle language '{SUBTITLE_LANGUAGE}'")
        print("  Supported languages: ja (Japanese), en (English), zh (Chinese)")
        sys.exit(1)
    
    # Check dependencies
    if not check_dependencies():
        print("Please install missing dependencies first, then run the program again.")
        sys.exit(1)
